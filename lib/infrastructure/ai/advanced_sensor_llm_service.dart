...
class AdvancedSensorLLMService {
  static final AdvancedSensorLLMService _instance =
  ...
  static const String _edgeModel =
      'meta/llama-3.1-8b-instruct'; // For edge inference
...
- Amostras: ${values.length}''';
  ...
  void bufferSensorData(SensorData data) {
    ...
    final cutoffTime = DateTime.now().subtract(
      Duration(milliseconds: _slidingWindowMs),
    );
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  '''Você é um especialista em análise de sensores IoT em tempo real.
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  '''Você é um especialista em fusão multimodal para análise contextual avançada.
            ...
            {'role': 'user', 'content': modalityDescriptions.toString()},
          ],
          'temperature': 0.7,
          'max_tokens': _maxTokens,
        },
  ...
  Future<Map<String, dynamic>> performEdgeInference(
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  'Analise rápida de sensores. Responda brevemente com: activity, alert (se houver), confidence.',
            },
            {'role': 'user', 'content': compactDescription},
          ],
          'temperature': 0.5,
    ...
    grouped.forEach((sensor, values) {
      if (values.isNotEmpty) {
        ...
        if (values.length > 2) {
          final skewness =
              values
                  .map((v) => pow((v - mean) / sqrt(variance), 3))
                  .reduce((a, b) => a + b) /
              values.length;
  ...
  List<SensorData> adaptiveSample(List<SensorData> data, double activityLevel) {
    ...
    final samplingRate = activityLevel > 0.7
        ? 1
        : activityLevel > 0.3
        ? 2
        : 4;
