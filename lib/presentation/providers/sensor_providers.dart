...
final sensorDataHistoryProvider = StateNotifierProvider.family<
...
class SensorDataHistoryNotifier extends StateNotifier<List<SensorData>> {
  ...
  SensorDataHistoryNotifier(this.sensorType, {this.maxHistory = 50}) : super([]);
...
class SensorAnalytics {
  ...
  String classifyActivity(List<AccelerometerData> data) {
    ...
    final avgMagnitude = data.map((d) => d.magnitude).reduce((a, b) => a + b) / data.length;
  ...
  String classifyEnvironment(List<LightData> data) {
    ...
    final avgLux = data.map((d) => d.luxValue).reduce((a, b) => a + b) / data.length;
  ...
  int calculateBatteryHealthScore(List<BatteryData> data) {
    ...
    final avgLevel = recent.map((d) => d.batteryLevel).reduce((a, b) => a + b) / recent.length;
    final chargingRatio = recent.where((d) => d.isCharging).length / recent.length;
  ...
  Map<String, dynamic> getSensorStatistics(List<SensorData> data) {
    ...
    grouped.forEach((sensorType, sensorData) {
      stats[sensorType] = {
        ...
        'duration': sensorData.last.timestamp.difference(sensorData.first.timestamp).inMinutes,
      };
    });
